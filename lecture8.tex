...
\textbfit{Теорема} (Интегральная формула Коши). Пусть $f \in \mathcal{O} (D),\  G \ -$ ограниченная область, $\overline{G} \subset D, \ \partial G $
...
\[\ \frac{f(z)}{z-a} \in \mathcal{O}(G_r) \overset{\text{ИТК}}{\Rightarrow} \oint_{\partial G_r^+} \frac{f(z)}{z-a} dz = 0 = \oint_{\partial G^+}\frac{f(z)}{z-a}dz + \oint_{(|z-a|=r)^-} \frac{f(z)}{z-a}dz \text{, т.е. }\] 
...
\[\leq \frac{\sup_{|z-a|=r}\left|f(z)-f(a)\right|}{r} \cdot 2\pi r < \epsilon \ \forall r \underset{r\to 0}{<} \epsilon \delta, \text{ т.к. }f(z) \in \mathcal{O}(G) \Rightarrow \sup_{|z-a|=r}|f(z)-f(a)| \underset{r\to 0}{\to} 0\]
\[\text{Но т.к. интеграл не зависит от $r$, то } \oint_{(|z-a|=r)^+}\frac{f(z)}{z-a}dz = 2\pi if(a) = \oint_{\partial G^+} \frac{f(z)}{z-a}dz\blacktriangleleft\]
...
\end{array}\right) \left(\begin{array}{ll}
    x'(t) \\
    y'(t)
\end{array}\right) = 0 \Rightarrow g \equiv const \Rightarrow \Phi(a) = \Phi(b) \text{ -- противоречие } \blacktriangleleft\]
...
\textbfit{Теорема} (о существовании первообразной в круге). Пусть $f \in C(U_R (a)) \ \forall \overline{\triangle} \subset U_R(a)$ $\ \int_{\partial \triangle} f(z) dz = 0$. Тогда у $f \ \exists$ первообразная в $U_R(a).$
...
\[\oint_{\partial \triangle} f(\zeta) d\zeta = \int_{[a, z]} f(\zeta) d\zeta + \int_{[z, z+h]} f(\zeta) d\zeta  - \int_{[a, z+h]} f(\zeta) d\zeta\]
...
\textbfit{Теорема} (критерий существования первообразной в области). Пусть $f \in C(D).$ Тогда у $f$ в $D \ \exists$ первообразная $\Leftrightarrow \forall$ замкнутого пути $\Gamma:[a, b]\to D \ \oint_\Gamma f(z) dz = 0$
...
\[\text{Дальше, как в пред теореме } \blacktriangleleft\]
...
\[\text{По ИТК для односвязных областей } \oint_\Gamma f(z) dz = 0 \Rightarrow \text{ по критерию $\exists$ первообразная в $D$. } \blacktriangleleft\]