...
\section{Лекция 1. Комплексные числа. Операции над ними. Стереографическая проекция}\label{lection1}
...
\[z = |z| \left(\frac{x}{|z|} + i\frac{y}{|z|}\right) = |z|(\cos \phi + i \sin \phi)\]

\textbfit{Опр.} Число $\phi$ называют аргументом числа $z$, если $\begin{cases}\cos \phi = \frac{x}{|z|} \\ \sin \phi = \frac{y}{|z|}\end{cases}$

\textbfit{Опр.} $Arg \ z = \{\phi \in \mathbb{R} | \phi = \arg z\}$

Для 0 аргумент не определён.

\textbfit{Обозначение} (Эйлера): $e^{i\phi} := \cos \phi + i \sin \phi$ (на данный момент считаем картинкой, не верим, что это получено из ряда Тейлора)

\textbfit{Утв.} $e^{i(\phi_1+\phi_2)} = e^{i\phi_1} e^{i\phi_2}$
\[\triangleright \text{ Простая проверка по определению даёт формулу. } \blacktriangleleft\]

\textbfit{Утв.} (Формула Муавра) $(e^{i\phi})^n = e^{in\phi}$

\textbfit{Утв.} $z = |z| e^{i\phi}$ (полярный вид числа), $\overline{z} = |z| e^{-i\phi}$

\textbfit{Утв.} $z_1 = |z_1| e^{i\phi_1}, \ z_2 = |z_2| e^{i\phi_2} \Rightarrow \ z_1 z_2 = |z_1| \cdot |z_2| e^{i(\phi_1+\phi_2)}, \ \frac{z_1}{z_2} = \frac{|z_1|}{|z_2|} e^{i(\phi_1-\phi_2)}$
\[\triangleright \ z_2^{-1} = \frac{x_2}{x_2^2+y_2^2} - i \frac{y_2}{x_2^2+y_2^2} =\frac{1}{|z_2|^2} \overline{z_2} = \frac{1}{|z_2|^2} \cdot |z_2| e^{-i\phi_2} \blacktriangleleft\]
\[z^n = |z|^n e^{in\phi}\]
...
\[\xi^2 + \eta^2 + \left(y - \frac{1}{2}\right)^2 = \frac{1}{4} \text{ -- сфера Римана}\]
