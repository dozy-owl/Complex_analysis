...
\textbfit{Утв.} (О связи вычета с коэффициентами ряда Лорана). Пусть $a$ -- ИОТ $f(z), \ f \in \mathcal{O}(\mathring{U}_\epsilon(a)), f(z) = \sum_{n=-\infty}^{+\infty} c_n (z-a)^n$ в $\mathring{U}_\epsilon(a)$. Тогда $\underset{a}{\res} f = c_{-1}.$
\[\triangleright \ \underset{a}{\res} f = \frac{1}{2\pi i} \oint_{|z-a|=r} \sum_{n=-\infty}^{+\infty} c_n(z-a)^n dz \overset{|z-a|=r \text{комп}}{=} \frac{1}{2\pi i} \sum_{n=-\infty}^{+\infty} c_n \oint_{|z-a|=r} (z-a)^n dz = \frac{1}{2\pi i}c_{-1}\cdot 2\pi i \blacktriangleleft\]

\textbf{Формулы для вычисления вычетов:}
...
\textbfit{Опр.} Пусть $f \in \mathcal{O}(|z| > R).$ Тогда вычетом $f$ в $\infty$ называется число $\underset{\infty}{\res} f = -\frac{1}{2\pi i} \oint_{|z| = \tilde{R}} f(z) dz, \tilde{R} > R.$
...
\textbf{Формулы для вычетов в $\infty$}

1) $\infty$ -- УОТ. Тогда $f(z) = c_0 + \frac{c_{-1}}{z} + \sum_{n=2}^\infty \frac{c_{-n}}{z^n}.$
\[\lim_{z\to \infty}f(z) =: f(\infty)\]
\[\underset{\infty}{\res} f = -\lim_{z\to \infty} z(f(z) - f(\infty)) = \lim_{z\to \infty} z(f(\infty) - f(z))\]
...
\textbfit{Теорема} (О полной сумме вычетов). Пусть $f \in \mathcal{O}(\mathbb{C} \setminus \{a_1\ldots a_n\})$. Тогда $\underset{\infty}{\res} f + \sum_{k=1}^n \underset{a_k}{\res} f(z) = 0$.
...
\[\triangleright \ \left|\int_{\gamma_R} f(z) e^{i\lambda z}dz\right| \leq \left|\begin{array}{ll}
    z = R(\cos t + i\sin t) \\
