...
\[f_n(\Gamma(t))\Gamma'(t) \rightrightarrows f(\Gamma(T))\Gamma(t)\]
\[\underset{t \in [a, b]}{\sup} \left|(f_n(\Gamma(t))-f(\Gamma(t)))\Gamma'(t)\right| \leq \underset{t \in [a, b]}{\sup} |\Gamma'(t)|\cdot \underset{t \in [a, b]}{\sup} |f_n(\Gamma(t)) - f(\Gamma(t))| = \underset{t \in [a, b]}{\sup} |\Gamma'(t)|\times\]
\[\times \underset{z \in \gamma}{\sup} |f_n(z) - f(z)| \to 0\]
\[\int_\gamma f_n(z) dz = \int_a^b f_n(\Gamma(t)) \Gamma'(t) dt \to \int_a^b f(\Gamma(t))\Gamma'(t) dt = \int_\gamma f(z) dz \blacktriangleleft\]

\subsection{Формула Тейлора}
\textbfit{Теорема} Тейлора. 

Пусть $f \in \mathcal{O}(D), U_R(a) \subset D$. Тогда $c_n = \frac{1}{2\pi i} \oint_{|\zeta - a|=r}\frac{f(\zeta)}{(\zeta-a)^{n+1}}d\zeta, 0<r<R$
...
\[\triangleright \ 1) \text{ Покажем, что $c_n$ tн зависят от $r$. } 0<r_1<r_2<R\]
...
% \[\triangleright c_n = \frac{1}{2\pi i} \oint_{|\zeta-a|=r} \frac{f(\zeta)}{(\zeta-a)^{n+1}}d\zeta = \frac{1}{2\pi} \oint_{|zeta-a|=r} \frac{1}{(\zeta-a)^{n+1} \left(\sum_{k=0}^\infty b_k(\zeta-a)^k\right) d\zeta =\]
...
\[(*) = \frac{1}{2\pi} \sum_{k=0}^\infty b_k \oint_{|zeta-a|=r} (\zeta-a)^{k-n-1}d\zeta = \begin{cases}
    0, & k-n-1 \neq -1 \\
    2\pi i, & k - n - 1 = -1
\end{cases} = \frac{1}{2\pi} b_n \cdot 2\pi i = b_n \blacktriangleleft\]