...
\[\rho(z_n, z) = \frac{|z_n-z|}{\sqrt{1+|z_n|^2}\sqrt{1+|z|^2}} \geq \frac{|z_n-z|}{1+R^2}, \ \text{т.к. } |z_n| \leq R, \ |z| \leq R\]
...
\textbfit{Утв.} $z_n = x_n +iy_n, \ z=x+iy, \ z_n = r_n e^{i\phi_n}, \ z=re^{i\phi}, \ r_n>0, \ r>0$
...
3) $r_n \to r, \ \phi_n \to \phi \Rightarrow z_n \to z$

4) $z_n \to z, \ z \neq 0 \Rightarrow r_n \to r, \ \exists \phi_n = \arg z_n, \ \exists \phi = \arg z: \phi_n \to \phi$
\[\triangleright (1)-(2) \text{ очевидно}\]
\[(3): \ z_n = r_n (\cos \phi_n + i\sin \phi_n)\] 
\[x_n = r_n \cos \phi_n \to r\cos \phi, \ y_n = r_n \sin \phi_n \to r\sin \phi \overset{(1)}{\Rightarrow} z_n \to z\]
...
\[\arg z_n \in \left(-\frac{\pi}{2}, \frac{\pi}{2}\right), \ \phi_n = \arctg \frac{y_n}{x_n} \to \arctg \frac{y}{x} \in Arg(z)\]
...
\[\triangleright \left(1+\frac{x}{n} + i\frac{y}{n}\right)^n\]
...
\[= e^{\frac{n}{2}\ln\left(1+\frac{2x}{n}+\frac{x^2+y^2}{n^2}\right)} = e^{\frac{n}{2}\left(\frac{2x}{n}+\overline{o}\left(\frac{1}{n}\right)\right)} \to e^x\]
\[1+\frac{x}{n} \text{ начиная с какого-то номера лежит в правой полуплоскости}\]
\[\phi_n = \arg\left(1+\frac{x}{n} + i\frac{y}{n}\right) = \arctg \frac{\frac{y}{n}}{1+\frac{x}{n}} = \arctg \frac{y}{n+x}\]
\[\arg \left(1+\frac{x}{n}+i\frac{y}{n}\right)^n = n \cdot \phi_n = n\arctg \frac{y}{n+x} = n\left(\frac{y}{n+x} + \overline{o}\left(\frac{1}{n}\right)\right) \to y \blacktriangleleft\]
...
\textbfit{Утв.} Если $z = re^{i\phi}\neq 0$, то $\Ln z = \ln r + i\phi + 2\pi i k, k \in \mathbb{Z}$ 
\[\triangleright \ w = u+iv, \ e^w = e^u e^{iv} = re^{i\phi} = z \Leftrightarrow e^u = r, \ v = \phi +2\pi k, k \in \mathbb{Z} \blacktriangleleft\]

\textbfit{Опр.} $z^w :+ e^{w\Ln z}, z \neq 0$

\textbfit{Утв.} $z^n = \prod_{i=1}^n z, \ z^{\frac{1}{n}} = \sqrt[n]{z}$
\[\triangleright \ 1) z^n = e^{n \ln z} = e^{n(\ln|z|+i\phi+2\pi i k)} = e^{n\ln |z|}e^{in\phi}e^{i2\pi kn} = \prod_{i=1}^n z\]
\[2) e^{\frac{1}{n}}=e^{\frac{1}{n} (\ln |z| + i\phi + 2\pi ik)} = e^{\frac{1}{n}\ln|z|}e^{i\frac{\phi+2\pi k}{n}} = \sqrt[n]{z}, \ k \in \mathbb{Z} \blacktriangleleft\]

$A\cdot z = |A| e^{i\arg A}|z|e^{i\arg z}$

$\mathbb{R}^2\to\mathbb{R}^2$
\[|A|\left(\begin{array}{ll}\cos \phi & -\sin \phi \\
\sin \phi & \cos \phi \end{array}\right)\left(\begin{array}{ll} x \\ y    
\end{array}\right), \ \phi = \arg A\]
...
\[A=u_x(x_0, y_0), B = u_y(x_0, y_0), C = v_x, D = v_y\]
\[f(z_0-\Delta z)-f(z_0)=A\Delta x+B\Delta y + i(C\Delta x + D\Delta y)+\overline{\Delta z} = (*)\]
\[\Delta x = \frac{\Delta z + \Delta \overline{z}}{2i}, \ \Delta y = \frac{\Delta z - \Delta \overline{z}}{2}\]
\[(*) = \frac{\Delta z}{2}(A -iB+iC+D)+\frac{\Delta \overline{z}}{2}(A+iB+iC-D) + \overline{o}(\delta z)\]
\[\frac{\delta f}{\delta z} = \frac{\Delta z}{2}(A -iB+iC+D)\]
\[\frac{\delta f}{\delta \overline{z}} = \frac{\Delta \overline{z}}{2}(A+iB+iC-D)\]
